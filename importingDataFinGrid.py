    ...
    for attempt in range(1, maxRetries+1):
        try:
            ...
            if response.status_code == 200:
                ...
                logging.warning(f"Attempt {attempt} failed with status {response.status_code} for extracting fingrid dataset")
        except Exception as e:
            logging.error(f"Attempt {attempt} failed with error: {str(e)} for extracting fingrid dataset")
        sleep(2 ** attempt)
    raise Exception("All retries failed for data fetch")

try:
    logging.info(f"Starting ingestion for {startTimeExtraction} → {endTimeExtraction} for extracting fingrid dataset")
    data = fetch_data()
    csv_content = data["data"]
    
    new_rows = list(csv.DictReader(io.StringIO(csv_content), delimiter=';'))
    if not new_rows:
        logging.warning("No new data to ingest in fingrid dataset")
    else:
        ...
        logging.info(f"Ingestion complete for extracting fingrid dataset. Rows written: {data['pagination']['to']}")
except AssertionError as schema_error:
    logging.error(f"Schema validation failed: {schema_error} in fingrid extraction")
except Exception as e:
    logging.error(f"Critical failure: {e} in fingrid extraction")